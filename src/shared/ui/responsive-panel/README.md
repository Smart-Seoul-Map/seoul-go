...
- `onOpenChange(open, { reason })`는 trigger, closeButton, escapeKeyDown, interactOutside, drag, handleClickOnLastSnapPoint를 구분한다.
...
- 스냅 포인트에는 `showHandle`을 함께 사용한다. 핸들 클릭으로 다음 높이로 이동하고 마지막 높이에서는 닫는다.
- 드래그는 세로 이동 의도를 확인한 뒤 시작하고, 가장 가까운 스냅에 정착한다. 최저 높이 절반 아래로 내리면 닫는다.
