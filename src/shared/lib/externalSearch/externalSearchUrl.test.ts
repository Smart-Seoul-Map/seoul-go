...
describe("외부 검색 URL", () => {
  ...
  test("공백과 특수문자가 섞인 검색어도 안전하게 인코딩한다", () => {
    expect(buildExternalSearchUrl("NAVER", " 서울 숲 & 공원 ")).toBe(
      "https://search.naver.com/search.naver?query=%EC%84%9C%EC%9A%B8%20%EC%88%B2%20%26%20%EA%B3%B5%EC%9B%90"
    );
  });
  ...
  test("새 탭으로 검색 결과를 연다", () => {
    const open = vi.spyOn(window, "open").mockReturnValue(null);

    expect(openExternalSearch("NAVER", "서울숲")).toBe(true);
  ...
  test("검색어가 없으면 창을 열지 않는다", () => {
    ...
    expect(openExternalSearch("NAVER", " ")).toBe(false);
